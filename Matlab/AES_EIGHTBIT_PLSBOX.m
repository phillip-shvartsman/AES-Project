...
for i=1:176
    ...
    if (mod(9+i,16) == 0||mod(5+i,16)==0||mod(i+1,16)==0||mod(i-2,16)==0)&&i>2
        shifted = shiftRow([storeComb(4),storeComb(5),storeComb(6),storeComb(7)],shiftController);
        %New shift controller output after every shift
        shiftController = shiftController - 1;
        storeComb(4:7) = shifted;
    ...
    if (mod(i-2,16)==0||mod(i-3,16)==0||mod(i,16)==0||mod(i-1,16)==0)&&i>4&&i<150
        mixed = mixColumns(gf([storeComb(4),storeComb(8),storeComb(12),storeComb(16)],8,283));
       
        storeComb(4) = mixed(1);
        storeComb(8) = mixed(2);
        storeComb(12) = mixed(3);
        storeComb(16) = mixed(4);
    end
